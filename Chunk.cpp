...
Chunk::Chunk(std::shared_ptr<CubeShader> shader, const glm::vec3& position)
    ...
    for (int i = 0; i <= DIR_LAST; ++i) {
        ...
        for (unsigned j = 0; j < 2; ++j) {
            ...
            glVertexAttribPointer(attrib[j], VertPosTexCompressed::Attributes[j].size,
                                  VertPosTexCompressed::Attributes[j].type,
                                  VertPosTexCompressed::Attributes[j].normalized, vSize,
                                  VertPosTexCompressed::Attributes[j].offset);
...
void Chunk::UpdateVertexes() {
    ...
    for (auto it = m_chunkData.begin(); it != end; ++it) {
        ...
        } else if (m_Neighbours[DIR_BACK] == nullptr
                || !m_Neighbours[DIR_BACK]->isSolid(x, y, Chunk::ms_chunkSize.z - 1)) {
            fl += VSBL_FCS_BACK;
            tex[DIR_BACK] = it->second;
        }
        ...
        } else if (m_Neighbours[DIR_LEFT] == nullptr
                || !m_Neighbours[DIR_LEFT]->isSolid(Chunk::ms_chunkSize.x - 1, y, z)) {
            fl += VSBL_FCS_LEFT;
            tex[DIR_LEFT] = it->second;
        }
